...
атомарны, а уникальный ключ outbox не допускает дубликатов. После commit worker будит NotificationWorker, который только
отправляет готовые сообщения и обновляет статус доставки. После пяти ошибок материализации destination остаётся
`pending`: порог вызывает alert в логе, но повторы продолжаются с backoff. Внешнего брокера нет: обе очереди находятся
в SQLite.
...
Уникальный ключ таблицы состоит из event_type, event_id, chat_id, message_thread_id и part_index. audience в ключ не
входит, но разные отчёты используют разные event_type.
