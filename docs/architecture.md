...
Событийная часть проекта является внутренней и оперативной: доменные события представлены типом std::variant и живут в
памяти. Отдельный event log не ведётся, но намерение отправить уведомление сохраняется в локальной outbox-очереди.
...
8. создаёт API-клиенты, репозитории, форматтеры, NotificationService, NotificationWorker и EventDispatcher;
9. запускает поток синхронизации ClanManager;
10. запускает поток NotificationWorker для фоновой доставки outbox-сообщений;
11. запускает поток TelegramBotService;
12. ждёт сигнал остановки и последовательно останавливает все три потока.

Остановка через stop(), requestStop() и stopLoop() будит условные переменные, поэтому приложение не обязано ждать
окончания 15-минутного интервала или периода ожидания worker-а. Уже выполняющийся сетевой запрос завершается отдельно,
...
отдельные соединения используются для синхронизации, Telegram-команд и NotificationWorker. При открытии включаются:
...
Сервисы формируют события после записи данных, а EventDispatcher последовательно вызывает NotificationService. Сервис
форматирует сообщения и сохраняет намерение доставки в SQLite. NotificationWorker отдельно читает готовые записи и
отправляет их в Telegram. Внешнего брокера нет: outbox является локальной устойчивой очередью.
...
Слой отчётов разделён на форматтеры, постановку в очередь и доставку:

- форматтер получает событие, при необходимости читает дополнительные данные из репозиториев и возвращает HTML-текст;
- NotificationService выбирает форматтер, аудиторию и список Telegram-назначений, затем атомарно создаёт outbox-записи;
...
NotificationService использует NotificationRepo для атомарной постановки в очередь, а NotificationWorker — для доставки и
переходов статусов:
