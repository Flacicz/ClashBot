...
- сохраняет уведомления в устойчивой outbox-очереди до отправки в Telegram;
...
При запуске приложение открывает SQLite, применяет неприменённые миграции, загружает каталог гайдов и запускает три
фоновых потока:
...
5. `NotificationService` строит сообщения форматтерами и атомарно добавляет их в таблицу `notifications` со статусом
   `pending`.
6. `NotificationWorker` читает готовые pending-сообщения и отправляет их через `TelegramNotifier`. После успеха он
   устанавливает `sent`, при временной ошибке оставляет `pending` с `next_attempt_at`, а при окончательной — `failed`.
7. `TelegramBotService` получает входящие обновления через long polling.
8. После цикла синхронизации менеджер ждёт 15 минут. При остановке ожидание прерывается сразу.
